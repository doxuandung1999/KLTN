...
async function createExpressApp()
{
	...
	expressApp.post(
		'/api/intergrates/register', async (req, res, next) =>
		{
			try
			{
				axios
					.post(`${baseURL}intergrates/register`, req.body)
					.then((result) => 
					{
	...
	expressApp.post(
		'/api/intergrates/login', async (req, res, next) =>
		{
			try
			{
				axios
					.post(`${baseURL}intergrates/login`, req.body)
					.then((result) => 
					{
						res.status(200).json(result.data);
					})
	...
	expressApp.get(
		'/api/intergrates/token', async (req, res) =>
		{
			try
			{
				axios.get(`${baseURL}intergrates/token?sid=${req.params.res.sid}`)
					.then((result) => 
					{
						res.status(200).json(result.data);
					})
	...
	expressApp.get(
		'/api/intergrates/users/me', async (req, res) =>
		{
			try
			{
				const reqContent = {
					method  : 'get',
					url     : `${baseURL}intergrates/users/me`,
					headers : {
	...
	expressApp.get(
		'/api/intergrates/users/authen', async (req, res) =>
		{
			try
			{
				const reqContent = {
					method  : 'get',
					url     : `${baseURL}intergrates/users/authen`,
					headers : {
						Authorization  : req.headers.authorization,
						'Content-Type' : 'application/json'
					}
	 ...
	 */
	expressApp.post(
		'/rooms/share/:roomId', upload.single('file'), (req, res) => 
		{
			res.json({ 'message': 'Done' });
		});

	/**
	 * API GET shared resource
	 * the room.
	 */
	expressApp.get(
		'/download/:roomId/:filename', (req, res) => 
		{
			const file = `./shared_files/${ req.params.roomId }/${ req.params.filename}`;
